...
def release_project(cwdir, project_name, args):
    ...
    if is_linux and (archive_format == "appimage" or archive_format == None):
        make_appimage(cwdir, release_workspace, project_name, app_version)
...
def make_appimage(cwdir, release_workspace, project_name, app_version):
    # save as an appimage format
    os.mkdir(os.path.join(release_workspace))
    os.mkdir(os.path.join(release_workspace, "usr"))
    bin_dir = os.path.join(release_workspace, "usr", "bin")
...
"""
    ...
    if os.path.exists(local_appimage_path):
        try:
            check_output([local_appimage_path, "-l", "-v", release_workspace], stderr=STDOUT)
        except CalledProcessError as err:
